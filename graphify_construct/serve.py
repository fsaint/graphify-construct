...
from graphify_construct.security import sanitize_label, check_graph_file_size_cap
from graphify_construct.build import edge_data
from graphify_construct import temporal as _temporal
...
def serve(graph_path: str = "graphify-out/graph.json") -> None:
    ...
    async def list_tools() -> list[types.Tool]:
        return [
            ...
            types.Tool(
                name="tasks_due_before",
                description="Return task nodes with due_date on or before the given date and not yet completed.",
                inputSchema={
                    "type": "object",
                    "properties": {
            ...
            types.Tool(
                ...
                description="Return upstream and/or downstream tasks from a given task node.",
                inputSchema={
                    "type": "object",
                    "properties": {
                        "task_id": {"type": "string", "description": "Node ID of the task"},
            ...
            types.Tool(
                ...
                description="Return tasks and documents linked to an entity via responsible_for edges.",
                inputSchema={
                    "type": "object",
                    "properties": {
                        "entity_id": {"type": "string", "description": "Node ID of the person or institution"},
                    },
                    "required": ["entity_id"],
                },
            ),
            types.Tool(
                name="critical_path",
                description="Return the critical path through all task nodes and the total duration in days.",
                inputSchema={"type": "object", "properties": {}},
            ),
        ]
    ...
    def _tool_tasks_due_before(arguments: dict) -> str:
        import datetime
        date_str = arguments["date"]
        try:
            when = datetime.date.fromisoformat(date_str)
        except (ValueError, TypeError):
            return json.dumps({"error": f"Invalid date: {date_str!r}"})
        results = _temporal.tasks_due_before(G, when)
        return json.dumps(results, default=str)

    def _tool_dependency_chain(arguments: dict) -> str:
        task_id = arguments["task_id"]
        direction = arguments.get("direction", "both")
        chain = _temporal.dependency_chain(G, task_id, direction=direction)
        return json.dumps({"task_id": task_id, "direction": direction, "chain": chain})

    def _tool_responsible_for(arguments: dict) -> str:
        entity_id = arguments["entity_id"]
        results = []
        for u, v, data in G.edges(data=True):
            if data.get("relation") == "responsible_for":
                if u == entity_id:
                    results.append({"id": v, "label": G.nodes[v].get("label", v) if v in G else v})
                elif v == entity_id:
                    results.append({"id": u, "label": G.nodes[u].get("label", u) if u in G else u})
        return json.dumps({"entity_id": entity_id, "responsible_for": results})

    def _tool_critical_path(arguments: dict) -> str:
        dag = _temporal.build_dependency_dag(G)
        path, total_days = _temporal.critical_path(dag)
        return json.dumps({"path": path, "total_days": total_days})
    ...
    _handlers = {
        ...
        "tasks_due_before": _tool_tasks_due_before,
        "dependency_chain": _tool_dependency_chain,
        "responsible_for": _tool_responsible_for,
        "critical_path": _tool_critical_path,
    }
    ...
    async def read_resource(uri: AnyUrl) -> str:
        ...
        if uri_str == "graphify://surprises":
            try:
                from graphify_construct.analyze import surprising_connections
        ...
        if uri_str == "graphify://questions":
            try:
                from graphify_construct.analyze import suggest_questions
