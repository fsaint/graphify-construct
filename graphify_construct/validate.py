...
def _validate_and_clear_date(node: dict, field: str) -> None:
    """Warn and clear a node date field if it is not ISO-8601."""
    val = node.get(field)
    if val is None:
        return
    try:
        ...
        print(
            f"[graphify-construct] Warning: node {node.get('id', '?')!r} has "
            f"invalid {field}={val!r}; clearing field.",
            file=sys.stderr,
        )
        node[field] = None
...
def validate_extraction(data: dict) -> list[str]:
    ...
    if "nodes" not in data:
        ...
        for i, node in enumerate(data["nodes"]):
            ...
            for _date_field in ("start_date", "due_date", "completed_at"):
                _validate_and_clear_date(node, _date_field)
