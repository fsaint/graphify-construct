"""graphify-construct CLI — turn construction project documents into a queryable knowledge graph."""
...
try:
    ...
    __version__ = _pkg_version("graphify-construct")
except Exception:
    __version__ = "unknown"
...
def _enforce_graph_size_cap_or_exit(gp: Path) -> None:
    ...
    """
    from graphify_construct.security import check_graph_file_size_cap
    try:
...
def uninstall_all(project_dir: Path | None = None, purge: bool = False) -> None:
    ...
    # Git hook
    try:
        from graphify_construct.hooks import uninstall as hook_uninstall
...
def main() -> None:
    ...
    if cmd == "install":
        ...
        if len(sys.argv) < 3:
            ...
            sys.exit(1)
        from graphify_construct.affected import DEFAULT_AFFECTED_RELATIONS, format_affected, load_graph
        query = sys.argv[2]
        ...
        from graphify_construct.ingest import save_query_result as _sqr
        out = _sqr(
        ...
        if len(sys.argv) < 4:
            ...
            sys.exit(1)
        from graphify_construct.serve import _score_nodes
        from networkx.readwrite import json_graph
        import networkx as _nx
        ...
        segments = []
        from graphify_construct.build import edge_data
        ...
        if len(sys.argv) < 3:
            ...
            sys.exit(1)
        from graphify_construct.serve import _find_node
        from networkx.readwrite import json_graph
        label = sys.argv[2]
        graph_path = _default_graph_path()
        args = sys.argv[3:]
        ...
        from graphify_construct.diagnostics import (
            diagnose_file,
        ...
        if len(sys.argv) < 3:
            ...
            sys.exit(1)
        from graphify_construct.ingest import ingest as _ingest
        url = sys.argv[2]
        ...
        if not watch_path.exists():
            ...
            sys.exit(1)
        from graphify_construct.watch import watch as _watch
        try:
            _watch(watch_path)
        ...
        from graphify_construct.cluster import cluster, score_all, remap_communities_to_previous
        from graphify_construct.analyze import god_nodes, surprising_connections, suggest_questions
        from graphify_construct.report import generate
        from graphify_construct.export import to_json, to_html
        ...
        from graphify_construct.export import _git_head as _gh
        _commit = _gh()
        ...
        from graphify_construct.export import backup_if_protected as _backup
        _backup(out)
        ...
        if not watch_path.exists():
            print(f"error: path not found: {watch_path}", file=sys.stderr)
            sys.exit(1)
        from graphify_construct.watch import _rebuild_code
        ...
        if len(sys.argv) < 3:
            ...
            sys.exit(1)
        from graphify_construct.watch import check_update
        ...
        from graphify_construct.tree_html import write_tree_html, DEFAULT_MAX_CHILDREN
        ...
        import networkx as _nx
        from networkx.readwrite import json_graph as _jg
        from graphify_construct.build import prefix_graph_for_global as _prefix
        graphs = []
        ...
        subcmd = sys.argv[2] if len(sys.argv) > 2 else ""
        if subcmd not in ("html", "callflow-html", "obsidian", "wiki", "svg", "graphml", "neo4j", "gantt"):
            ...
            print("  gantt     [--graph PATH] [--output PATH]", file=sys.stderr)
            sys.exit(1)
        ...
        if subcmd == "callflow-html":
            from graphify_construct.callflow_html import write_callflow_html as _write_callflow_html
        ...
        from networkx.readwrite import json_graph as _jg
        from graphify_construct.build import build_from_json as _bfj
        ...
        if subcmd == "html":
            from graphify_construct.export import to_html as _to_html
            if no_viz:
            ...
            from graphify_construct.export import to_obsidian as _to_obsidian, to_canvas as _to_canvas
            ...
            from graphify_construct.wiki import to_wiki as _to_wiki
            from graphify_construct.analyze import god_nodes as _god_nodes
            if not communities:
                print(
            ...
            if neo4j_uri:
                ...
                from graphify_construct.export import to_cypher as _to_cypher
                _to_cypher(G, str(out_dir / "cypher.txt"))
                print(f"cypher.txt written - import with: cypher-shell < {out_dir}/cypher.txt")

        elif subcmd == "gantt":
            gantt_output = Path(_GRAPHIFY_OUT) / "gantt.html"
            extra = sys.argv[3:]
            i = 0
            while i < len(extra):
                if extra[i] == "--output" and i + 1 < len(extra):
                    gantt_output = Path(extra[i + 1]); i += 2
                elif extra[i] == "--graph" and i + 1 < len(extra):
                    graph_path = Path(extra[i + 1]); i += 2
                else:
                    i += 1
            from networkx.readwrite import json_graph as _jg
            _enforce_graph_size_cap_or_exit(graph_path)
            _G = _jg.node_link_graph(json.loads(graph_path.read_text(encoding="utf-8")))
            from graphify_construct.gantt_html import to_gantt_html
            to_gantt_html(_G, gantt_output)
            print(f"Gantt chart → {gantt_output}")

    elif cmd == "benchmark":
        from graphify_construct.benchmark import run_benchmark, print_benchmark
        ...
        subcmd = sys.argv[2] if len(sys.argv) > 2 else ""
        from graphify_construct.global_graph import (
        ...
        from graphify_construct.llm import (
        ...
        from graphify_construct.detect import (
            detect as _detect,
        ...
        if code_files:
            from graphify_construct.extract import extract as _ast_extract
        ...
        from graphify_construct.cache import (
        ...
        if no_cluster:
            ...
            if global_merge:
                from graphify_construct.global_graph import global_add as _global_add
        ...
        from graphify_construct.build import (
            build as _build,
            build_from_json as _build_from_json,
            build_merge as _build_merge,
        )
        from graphify_construct.cluster import cluster as _cluster, score_all as _score_all
        from graphify_construct.export import to_json as _to_json
        from graphify_construct.analyze import god_nodes as _god_nodes, surprising_connections as _surprising
        ...
        from graphify_construct.cache import check_semantic_cache
        if len(sys.argv) < 3:
