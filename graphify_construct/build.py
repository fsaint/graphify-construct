...
# emit. Maps construction-adjacent synonyms to canonical types and falls back
# to "document" for any other invalid value (see #840).
_FILE_TYPE_SYNONYMS = {
    "contractor": "institution",
    "subcontractor": "institution",
    "company": "institution",
    "agency": "institution",
    "milestone": "deadline",
    "activity": "task",
...
def build_from_json(extraction: dict, *, directed: bool = False, root: str | Path | None = None) -> nx.Graph:
    ...
    for node in extraction.get("nodes", []):
        ...
        # Default missing/None file_type to "document" so legacy graph.json
        ...
        if ft and ft not in VALID_FILE_TYPES:
            node["file_type"] = _FILE_TYPE_SYNONYMS.get(ft, "document")
    ...
    # Materialise predecessors[] → precedes edges
    for node in extraction.get("nodes", []):
        nid = node.get("id")
        if not nid:
            continue
        for pred_id in (node.get("predecessors") or []):
            pred_id = _normalize_id(str(pred_id))
            # resolve through norm_to_id if possible
            resolved = norm_to_id.get(pred_id, pred_id)
            if resolved in G and nid in G:
                G.add_edge(resolved, nid,
                           relation="precedes",
                           confidence="EXTRACTED",
                           confidence_score=1.0,
...
def build(
    ...
    *,
    ...
    """
    from graphify_construct.dedup import deduplicate_entities
