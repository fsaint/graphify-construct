...
def mermaid_class_defs() -> list:
    ...
    return [
        ...
        "    classDef task fill:#1e3a5f,stroke:#3b82f6,color:#dbeafe,stroke-width:1px;",
        "    classDef person fill:#064e3b,stroke:#10b981,color:#d1fae5,stroke-width:1px;",
        "    classDef institution fill:#451a03,stroke:#f59e0b,color:#fef3c7,stroke-width:1px;",
        "    classDef deadline fill:#450a0a,stroke:#ef4444,color:#fee2e2,stroke-width:1px;",
        "    classDef permit fill:#500724,stroke:#ec4899,color:#fce7f3,stroke-width:1px;",
    ]
...
def _suggest_tag(label: str, file_type: str, lang: str, kind: str = "") -> str:
    ...
    if file_type == "task":
        return f'<span class="tag tag-func">{pick_text(lang, "任务", "Task")}</span>'
    if file_type == "person":
        return f'<span class="tag tag-class">{pick_text(lang, "人员", "Person")}</span>'
    if file_type == "institution":
        return f'<span class="tag tag-cmd">{pick_text(lang, "机构", "Institution")}</span>'
    if file_type == "deadline":
        return f'<span class="tag tag-async">{pick_text(lang, "截止日期", "Deadline")}</span>'
    if file_type == "permit":
        return f'<span class="tag tag-endpoint">{pick_text(lang, "许可证", "Permit")}</span>'
